...
package fptu.capstone.gymmanagesystemstaff.ui.attendance
...
import fptu.capstone.gymmanagesystemstaff.viewmodel.AttendanceViewModel

@OptIn(ExperimentalGetImage::class)
@Composable
fun AttendanceScreen(attendanceViewModel: AttendanceViewModel = hiltViewModel()) {
    val context = LocalContext.current
    val lifecycleOwner = LocalLifecycleOwner.current
    var qrCodeContent by remember { mutableStateOf("") }
    val previewView = remember { PreviewView(context) }
    val cameraPermissionState = rememberPermissionState(permission = Manifest.permission.CAMERA)
    val attendanceState by attendanceViewModel.trainerAttendance.collectAsState()
                                ...
                                barcodes.forEach { barcode ->
                                    barcode.rawValue?.let { qrCode ->
                                        if (qrCode != qrCodeContent) {
                                            ...
                                            attendanceViewModel.createAttendance(trainerId = qrCode.split("_").first(), slotId = qrCode.split("_").last())
                                        }
                                        imageProxy.close()
